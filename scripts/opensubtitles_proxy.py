...
ENV = _load_env()
PROXY_KEY = ENV.get("API_KEY", "")
LISTEN_HOST = ENV.get("OPENSUBTITLES_PROXY_HOST", "0.0.0.0")
LISTEN_PORT = int(ENV.get("OPENSUBTITLES_PROXY_PORT", "8876"))


class OpenSubtitlesBackend:
    def __init__(self):
        self.api_key = ENV.get("OPENSUBTITLES_API_KEY", "")
        self.username = ENV.get("OPENSUBTITLES_USERNAME", "")
        self.password = ENV.get("OPENSUBTITLES_PASSWORD", "")
        self.languages = ENV.get("OPENSUBTITLES_LANGUAGES", "en") or "en"
        self.prefer_sdh = ENV.get("OPENSUBTITLES_PREFER_SDH", "false").lower() == "true"
        self._token: Optional[str] = None
        self._base_url = "https://api.opensubtitles.com"
        self._auth_time = 0.0
        self._auth_ttl = 20 * 60
        self._headers = {
            ...
            "X-User-Agent": "SamAssist Proxy v1",
        }
    ...
    def download_subtitle(self, file_id: int, sub_format: str = "srt") -> tuple[bytes, str]:
        self.ensure_auth()
        payload = self._request_json(
        ...
        content = self._request_bytes(link)
...
BACKEND = OpenSubtitlesBackend()


class Handler(BaseHTTPRequestHandler):
    server_version = "SamAssistOpenSubtitlesProxy/1.0"
    ...
    def do_POST(self):
        ...
        try:
            if self.path == "/search":
                candidates = BACKEND.search_candidates(
                    title=str(payload.get("title") or ""),
                    year=payload.get("year"),
                    original_name=str(payload.get("original_name") or ""),
                    languages=str(payload.get("languages") or BACKEND.languages),
                )
                return self._send_json(200, {"candidates": candidates})

            if self.path == "/download":
                file_id = int(payload.get("file_id") or 0)
                sub_format = str(payload.get("sub_format") or "srt")
                content, filename = BACKEND.download_subtitle(file_id, sub_format=sub_format)
    ...
    def _authorized(self) -> bool:
        return bool(PROXY_KEY) and self.headers.get("X-Proxy-Key", "") == PROXY_KEY
